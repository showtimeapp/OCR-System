"""
Test client — send PDF to API, download results
"""
import requests, sys, time, json

API = "http://localhost:8000"

def extract(pdf_path, sync=True):
    """Send PDF and get results."""
    ...
    with open(pdf_path, 'rb') as f:
        ...
        if sync:
            ...
            t0 = time.time()
            resp = requests.post(f'{API}/extract', files=files, timeout=3600)
            dt = time.time() - t0
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print('Usage: python test_client.py <pdf_path> [--async]')
        sys.exit(1)

    pdf = sys.argv[1]
    is_async = '--async' in sys.argv
    extract(pdf, sync=not is_async)
